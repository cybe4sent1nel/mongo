...
Unbounded recursion in `mongo-go-driver`'s generic BSON-to-`interface{}` decode path (`emptyInterfaceCodec` ⇄ `dDecodeValue`/`mapCodec`) causes an uncatchable `fatal error: stack overflow` when decoding a single crafted BSON document from any untrusted byte source — `bson.Unmarshal` on a file/queue/message payload, or a non-standard/malicious/compromised server via `Cursor.Decode`/`Cursor.All`

## Important scope note, stated up front

**A genuine, unmodified `mongod`/`mongos` cannot itself be used to store and later return a document deep enough to trigger this crash** — MongoDB's server enforces its own document-nesting cap (rejects documents past roughly 200 levels) at write time. So the "ordinary query response from a standard server" framing does **not** hold on its own.

I also want to correct something rather than let it stand uncorrected: an earlier draft of this report cited `mongo-csharp-driver` report #3790290 (and its duplicate #3996918) as a precedent of MongoDB *accepting* this exact "malicious/compromised server or MITM" threat model. That's wrong — #3790290 itself was closed **Informative**, not accepted, despite carrying a High (7-8.9) severity rating (severity and disposition are independent; a report can be rated High and still be closed Informative). Combined with `bson-rust` report #3996627 (also Informative, same reasoning: "realistic exploitation scenarios require a MITM position or a fully compromised server... an already-compromised environment"), that's **three separate reports on this exact bug class, all closed Informative on this exact reasoning.** That is a consistent program position, not a fluke, and I'm not going to pretend otherwise.

Given that, the only framing in this report I'd actually stand behind as likely to be treated as in-scope is:

**No server or network involved at all**: any Go code that calls `bson.Unmarshal`/`NewDecoder(...).Decode()` on BSON bytes from a file, a message queue, an inter-service payload, an upload, or any other channel it does not fully trust. This needs no server, no MITM, and no "already-compromised environment" of any kind — it's the exact framing under which the sibling `mongo-tools`/`bsondump` finding in this pass is fully proven, and it is not the framing three prior reports on this bug class were rejected on.

The `Cursor.Decode`/`Cursor.All`-via-a-hostile-server angle is included below for completeness since it's still a real, reachable path through this same code, but I'd score it as **likely Informative** given the program's demonstrated, repeated position on this exact precondition — not a strong part of this submission.
...
**Primary impact — no server involved**: any Go application using `mongo-go-driver`'s `bson` package to decode BSON bytes it received from a file, a message queue, an inter-service payload, an upload, or any other channel it does not fully trust — into `bson.M`, `bson.D`, `interface{}`, or any composite containing them — can be crashed by a single such document. This requires no server connection, no network access, and no credentials at all; it is the exact same threat model already fully proven, with no caveats, in the sibling `mongo-tools`/`bsondump` finding from this same pass.

**Secondary impact — server-adjacent, likely Informative on this program's own track record**: for applications consuming this data via `Cursor.Decode`/`Cursor.All`, the same crash fires if the connected server is malicious, compromised, or reachable to an on-path attacker — but a genuine, unmodified `mongod` cannot itself be induced to store or return a document this deep (see the scope note above), and this exact precondition has already been the stated reason for closing three prior reports on this bug class (`mongo-csharp-driver` #3790290/#3996918, `bson-rust` #3996627) as Informative. I'm including it for completeness, not presenting it as the strong part of this submission.
