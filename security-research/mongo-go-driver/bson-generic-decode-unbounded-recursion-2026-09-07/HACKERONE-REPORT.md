...
**A genuine, unmodified `mongod`/`mongos` cannot itself be used to store and later return a document deep enough to trigger this crash** — MongoDB's server enforces its own document-nesting cap (rejects documents past roughly 200 levels) at write time. So the "ordinary query response from a standard server" framing does **not** hold on its own, and I don't want this discovered by a reviewer rather than disclosed by me. The two threat models that *do* apply, and that I believe make this a real finding rather than a restatement of "an already-compromised server can hurt you":

1. **No server or network involved at all**: any Go code that calls `bson.Unmarshal`/`NewDecoder(...).Decode()` on BSON bytes from a file, a message queue, an inter-service payload, an upload, or any other channel it does not fully trust — the exact framing under which the sibling `mongo-tools`/`bsondump` finding in this pass is fully proven, no server needed.
2. **A server that is malicious, compromised, or a MITM on an unencrypted/improperly-verified connection** — the same precondition already present in the accepted `mongo-csharp-driver` finding (#3790290/#3996918) and in the bytes libbson/BSON wire format generally; a real `mongod` opting out of its own limits, or an attacker who controls the bytes on the wire, is not bound by the server's own storage-time validation.

I'm not claiming a standard, unmodified `mongod` server will ever hand this back through ordinary use — it won't. I raised this here explicitly rather than let a reviewer catch me implying otherwise.
...
**Secondary impact — server-adjacent**: for applications consuming this data via `Cursor.Decode`/`Cursor.All`, the same crash fires if the connected server is malicious, compromised, or reachable to an on-path attacker (unencrypted connection, or TLS without proper certificate verification) — a genuine, unmodified `mongod` cannot itself be induced to store or return a document this deep (see the scope note above), so this half of the impact requires an already-hostile server or network position, structurally identical to the precondition already present in the accepted `mongo-csharp-driver` finding.
