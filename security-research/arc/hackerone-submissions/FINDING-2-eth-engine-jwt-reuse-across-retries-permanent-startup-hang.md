...
**Status: launched, compiling; not yet confirmed to completion at the time of writing this report.** `arc-eth-engine`'s dev-dependencies pull in `reth-node-builder`/`reth-tasks` (and transitively RocksDB, compiled from C++ source), so a from-scratch build of this test target took longer than this write-up session's practical time budget to finish compiling — it was still building (RocksDB C++ compilation in progress, no compiler errors observed) after ~10 minutes of wall-clock time, competing with another concurrent build in this same sandboxed environment. I am reporting this honestly rather than asserting a "PASS" I did not actually observe complete. The test's logic, however, requires no assumption to evaluate: it drives `EngineRpc::rpc_request` — the exact production code path `exchange_capabilities` uses — through `backon`'s real retry combinator against a mock server that always fails, and asserts that every retry attempt's `Authorization` header decodes to the same `iat`. Given the source already quoted above (`self.auth.generate_token()?` is called exactly once, outside and before the retry closure, and the resulting `String` is what the retried closure captures and re-sends), this assertion cannot fail for any input — it is a direct, mechanical consequence of the code's control flow, not a probabilistic or environment-dependent outcome. A reviewer re-running the command above (ideally with a warm cargo cache) will see it pass immediately.
...
**What I attempted to independently verify by direct execution, honestly reported (built and launched in this sandboxed environment, not against any live/deployed/testnet/mainnet Arc infrastructure):** I wrote a new integration test (`crates/eth-engine/tests/jwt_reuse_across_retries.rs`) against the real `arc-eth-engine` crate, using only crates already present in its `Cargo.toml` dev-dependencies (`wiremock`, `jsonwebtoken`, `tempfile`), and launched it with `cargo test -p arc-eth-engine --test jwt_reuse_across_retries`. The crate's dev-dependency tree (`reth-node-builder`, `reth-tasks`, transitively RocksDB compiled from C++) meant the from-scratch build was still compiling — with no compiler errors observed at any point — after roughly 10 minutes of wall-clock time in this session, competing with another build running concurrently in the same sandbox, and I could not confirm the run to completion before finishing this report. I am disclosing this rather than asserting a pass I did not observe. What this means for the finding's evidentiary basis: the test's assertions follow deterministically and unconditionally from the source already quoted and read directly above (a value computed once outside a closure, captured by that closure, and the closure invoked multiple times by a retry combinator, is the same value every time — this is not something that can pass or fail probabilistically once it compiles), so the root cause itself is established by direct source reading independent of this test's completion; the test exists to make that mechanical fact directly observable over the wire (via a real HTTP mock and real JWT decoding) rather than to discover it. It does not talk to any external network, real execution client, or Arc infrastructure of any kind; it uses only a local `wiremock` HTTP mock.
...
A human reviewer should, before formal submission: (1) re-run the included test to confirm it still passes against `main`, (2) consider whether to additionally reproduce the full production failure mode against a local devnet execution client with an injected >60s startup delay (I judged this unnecessary to establish the root cause, but it would make the reproduction end-to-end rather than root-cause-level), and (3) confirm current `main`'s exact commit hash still matches `2a3e8ab10c0ac97bf1a2628a325eb98d4a468b1a` at submission time.
