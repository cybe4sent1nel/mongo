...
Correction on the record: an earlier draft of this report described the `mongo-csharp-driver` finding (#3790290/#3996918) as something MongoDB had "accepted." That's wrong — #3790290 was closed **Informative**, not accepted, despite a High (7-8.9) severity rating (severity and disposition are tracked independently).

Two distinct, independently valid reachability paths follow from the scope note above, presented on their own merits:

**Path A — no server involved.** `pymongo`'s `bson` package is a general-purpose, publicly documented codec (`bson.BSON(data).decode()`, `bson.decode(data)`), not something that exists only behind a live database connection. Decoding bytes from a file, a message queue, an inter-service payload, or any other untrusted channel is one of its ordinary, intended uses. This needs no server, no network, and no MITM — only the two preconditions already stated (CPython 3.10/3.11, and the process having raised `sys.setrecursionlimit()` at some point). It's the same threat model already fully proven, with no such caveat, in the sibling `mongo-tools`/`bsondump` finding from this same pass.

**Path B — via `Cursor.Decode`-style query consumption against a malicious/compromised/on-path-attacker-controlled server.** Worth naming plainly: `mongo-csharp-driver` #3790290/#3996918 and `bson-rust` #3996627 were both closed Informative on precisely this precondition — real program history on this exact bug shape. That doesn't necessarily mean this path is dead (disposition turns on how each report argues reachability, not just the abstract precondition), but I'm presenting Path A as the claim this report actually stands on, and Path B as additional, real, but higher-risk context.

## Impact

**Path A**: on CPython 3.10 or 3.11, any process that has ever raised `sys.setrecursionlimit()` above its real native-stack-safe threshold can be crashed by decoding a single crafted BSON document from a file, queue, or any other untrusted byte source — no server or network access required at all.

**Path B**: the same crash fires via `Cursor.Decode`-style query consumption if the connected server is malicious, compromised, or on-path-attacker-controlled — a genuine, unmodified `mongod` cannot itself deliver it (see the scope note above). Real, reachable impact through the same code, presented alongside Path A rather than in place of it.
